...
static inline void m_inv(const matrix src, matrix dest)
{
...
int make_unsp(int densit, int mode, int * num_dot, int cubus) {
  ...
  *num_dot=ndot; if (k) return 0;
  ...
  xus = xpunsp;
  for (l=0; l<ndot; l++) {
    i = MY_MAX((int) floor((1.+xus[3*l])/del_cube), 0);
    if (i>=ico_cube) i = ico_cube-1;
    j = MY_MAX((int) floor((1.+xus[1+3*l])/del_cube), 0);
    if (j>=ico_cube) j = ico_cube-1;
    k = MY_MAX((int) floor((1.+xus[2+3*l])/del_cube), 0);
		 ...
		 int index[],int ePBC,matrix box)
{
  ...
  real xmin=0, ymin=0, zmin=0, xMY_MAX, yMY_MAX, zMY_MAX, ra2MY_MAX, d, *pco;
...
////////////////////////////////////////////////////////////////
  ...
  if (mode & FLAG_VOLUME)
    vol=0.;
  if (mode & FLAG_DOTS) {
    MY_MAXdots = (3*n_dot*nat)/10;
    /* should be set to NULL on first user call */
      if(dots==NULL)
      {
          snew(dots,MY_MAXdots);
      }
      else
      {
          srenew(dots,MY_MAXdots);
      }
  ...
  ra2MY_MAX = radius[index[0]];
  for (iat_xx=1; (iat_xx<nat); iat_xx++) {
    iat = index[iat_xx];
    ra2MY_MAX = MY_MAX(ra2MY_MAX, radius[iat]);
  }
  ra2MY_MAX = 2*ra2MY_MAX;

  /* Added DvdS 2006-07-19 */
  /* Updated 2008-10-09 */
  if (box) {
    // set_pbc(&pbc,ePBC,box);
    set_pbc(&pbc, static_cast<PbcType>(ePBC), box);
...
#endif
    nxbox = MY_MAX(1,floor(norm(box[XX])/ra2MY_MAX));
    nybox = MY_MAX(1,floor(norm(box[YY])/ra2MY_MAX));
    nzbox = MY_MAX(1,floor(norm(box[ZZ])/ra2MY_MAX));
    if (debug)
      fprintf(debug,"nbox = %d, %d, %d\n",nxbox,nybox,nzbox);
  }
  else {
    /* dimensions of atomic set, cell edge is 2*ra_MY_MAX */
    iat  = index[0];
    xmin = coords[iat][XX]; xMY_MAX = xmin; xs=xmin;
    ymin = coords[iat][YY]; yMY_MAX = ymin; ys=ymin;
    zmin = coords[iat][ZZ]; zMY_MAX = zmin; zs=zmin;
    ra2MY_MAX = radius[iat];

    for (iat_xx=1; (iat_xx<nat); iat_xx++) {
      iat = index[iat_xx];
      pco = coords[iat];
      xmin = MY_MIN(xmin, *pco);     xMY_MAX = MY_MAX(xMY_MAX, *pco);
      ymin = MY_MIN(ymin, *(pco+1)); yMY_MAX = MY_MAX(yMY_MAX, *(pco+1));
      zmin = MY_MIN(zmin, *(pco+2)); zMY_MAX = MY_MAX(zMY_MAX, *(pco+2));
    ...
    zs = zs/ (real) nat;
    if (debug)
      fprintf(debug,"nsc_dclm: n_dot=%5d ra2MY_MAX=%9.3f %9.3f\n", n_dot, ra2MY_MAX, dotarea);

    d = xMY_MAX-xmin; nxbox = (int) MY_MAX(ceil(d/ra2MY_MAX), 1.);
    d = (((real)nxbox)*ra2MY_MAX-d)/2.;
    xmin = xmin-d; xMY_MAX = xMY_MAX+d;
    d = yMY_MAX-ymin; nybox = (int) MY_MAX(ceil(d/ra2MY_MAX), 1.);
    d = (((real)nybox)*ra2MY_MAX-d)/2.;
    ymin = ymin-d; yMY_MAX = yMY_MAX+d;
    d = zMY_MAX-zmin; nzbox = (int) MY_MAX(ceil(d/ra2MY_MAX), 1.);
    d = (((real)nzbox)*ra2MY_MAX-d)/2.;
    zmin = zmin-d; zMY_MAX = zMY_MAX+d;
  }
  /* Help variables */
  nxy = nxbox*nybox;
  nxyz = nxy*nzbox;
  ...
  if (box) {
    ...
    for (iat_xx=0; (iat_xx<nat); iat_xx++) {
      iat = index[iat_xx];
      pco = coords[iat];
      i = (int) MY_MAX(floor((pco[XX]-xmin)/ra2MY_MAX), 0);
      i = MY_MIN(i,nxbox-1);
      j = (int) MY_MAX(floor((pco[YY]-ymin)/ra2MY_MAX), 0);
      j = MY_MIN(j,nybox-1);
      l = (int) MY_MAX(floor((pco[ZZ]-zmin)/ra2MY_MAX), 0);
      l = MY_MIN(l,nzbox-1);
      i = i+j*nxbox+l*nxy;
      wkat1[iat_xx] = i;
      wkbox[i]++;
  ...
  /* MY_MAXnei = (int) floor(ra2MY_MAX*ra2MY_MAX*ra2MY_MAX*0.5); */
  MY_MAXnei = MY_MIN(nat, 27*j);
  snew(wknb,MY_MAXnei);
  for (iat_xx=0; iat_xx<nat; iat_xx++) {
    iat = index[iat_xx];
  ...
  for (iz=0; iz<nzbox; iz++) {
    ...
    if (box) {
      izs = iz-1;
      ize = MY_MIN(iz+2,izs+nzbox);
    }
    else {
      izs = MY_MAX(iz-1,0);
      ize = MY_MIN(iz+2, nzbox);
    }
    for (iy=0; iy<nybox; iy++) {
      ii = iy*nxbox+iii;
      if (box) {
	iys = iy-1;
	iye = MY_MIN(iy+2,iys+nybox);
      }
      else {
	iys = MY_MAX(iy-1,0);
	iye = MY_MIN(iy+2, nybox);
      }
	...
	if (box) {
	  ixs = ix-1;
	  ixe = MY_MIN(ix+2,ixs+nxbox);
	}
	else {
	  ixs = MY_MAX(ix-1,0);
	  ixe = MY_MIN(ix+2, nxbox);
	}
	iiat = 0;
	...
	for (iat=iii1; iat<iii2; iat++) {
	  ...
	  if (mode & FLAG_DOTS) {
	    for (l=0; l<n_dot; l++) {
	      if (wkdot[l]) {
		lfnr++;
		if (MY_MAXdots <= 3*lfnr+1) {
            MY_MAXdots = MY_MAXdots+n_dot*3;
            srenew(dots,MY_MAXdots);
		}
